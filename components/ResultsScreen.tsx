...
export default function ResultsScreen({
  ...
  const winner = useMemo(() => {
    ...
    players.forEach((player) => {
      player.votes.forEach((vote) => {
        if (scores[vote.entryId] !== undefined) scores[vote.entryId] += vote.points;
      });
    });
  ...
  const calculatedEntries = useMemo(() => {
    // don't execute if video is on
    if (showWinner && !videoEnded) return [];

    const scores: Record<string, { total: number; currentPointsToAdd: number | null }> = {};
    entries.forEach((e) => (scores[e.id] = { total: 0, currentPointsToAdd: null }));

    if (currentVoterId === null && !showWinner) {
      return entries.map((e) => ({ ...e, points: 0, currentPointsToAdd: null })).sort((a, b) => a.country.localeCompare(b.country));
    }

    sortedPlayers.forEach((player, index) => {
      if (showWinner) {
        player.votes.forEach((v) => { scores[v.entryId].total += v.points; });
      } else {
    ...
    return Object.entries(scores)
      .map(([id, data]) => ({ ...entries.find((e) => e.id === id)!, points: data.total, currentPointsToAdd: data.currentPointsToAdd }))
      .sort((a, b) => b.points - a.points);
  }, [entries, sortedPlayers, currentVoterId, currentVoterIndex, twelveRevealed, showWinner, videoEnded]);

  useEffect(() => {
    const pusher = new Pusher(process.env.NEXT_PUBLIC_PUSHER_KEY!, { cluster: process.env.NEXT_PUBLIC_PUSHER_CLUSTER! });
    const publicChannel = pusher.subscribe(`room-${roomCode}`);

    publicChannel.bind("voter-changed", (data: { currentVoterId: string | null }) => {
  ...
  useEffect(() => {
    ...
    if (isHost) {
      privateChannel.bind("client-emoji-tapped", (data: { playerId: string; emoji: string }) => {
        const now = Date.now();
        ...
        const container = document.getElementById(`avatar-container-${data.playerId}`);
      ...
      return (
        <div className="min-h-screen bg-black flex flex-col items-center justify-center text-white p-6 relative">
          ...
          <div className="z-10 flex flex-col items-center text-center mb-12 animate-in slide-in-from-bottom-10 duration-700">
            <span className="text-yellow-500 font-bold tracking-widest uppercase mb-2">The Winner Is</span>
            <h1 className="text-5xl font-black mb-2 text-center drop-shadow-lg">{winner?.country}</h1>
                ...
                <button
                  ...
                  onClick={() => {
                    if (currentPlayerId && privateChannelRef.current) {
                      ...
                      privateChannelRef.current.trigger("client-emoji-tapped", {
    ...
    return (
      <div className="relative min-h-screen w-full flex items-center justify-center text-white overflow-hidden bg-black">
        {iframeSrc ? (
          ...
          <div className="absolute inset-0 z-0 flex items-center justify-center bg-black">Brak wideo</div>
        )}
        <canvas
          ref={canvasRef}
          ...
          <div className="absolute top-6 right-6 z-30 bg-black/80 backdrop-blur-md border border-white/10 p-4 rounded-xl shadow-2xl flex flex-col w-64 animate-in fade-in duration-700">
            <div className="flex justify-between items-start mb-2 gap-2">
              <div className="overflow-hidden">
                <h2 className="text-yellow-500 text-[10px] font-bold tracking-widest uppercase mb-0.5">The Winner</h2>
                <h1 className="text-xl font-black text-white leading-tight truncate">{winner.country}</h1>
              </div>
            ...
            <p className="text-sm text-white/90 font-bold truncate w-full">{winner.artist}</p>
            <p className="text-xs text-pink-400 font-medium mb-4 truncate w-full">{winner.songTitle || winner.title}</p>
            <button
        ...
        <div className="absolute bottom-0 left-0 right-0 z-20 h-32 flex justify-center items-end pb-8 gap-6 md:gap-10 pointer-events-none animate-in fade-in slide-in-from-bottom-8 duration-700 delay-300">
          {players.map((player) => (
            <div key={player.id} id={`avatar-container-${player.id}`} className="relative flex flex-col items-center">
              <img src={player.avatar?.url || "/placeholder.png"} className="w-14 h-14 md:w-16 md:h-16 rounded-full border-2 border-white/30 shadow-xl object-cover z-10 relative" alt={player.name} />
              <span className="text-xs font-bold mt-2 text-white/90 bg-black/80 px-3 py-1 rounded-full border border-white/10 shadow-md">{player.name}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="h-screen w-full flex flex-col p-4 text-white overflow-hidden bg-black/40">
      ...
      <h1 className="text-3xl lg:text-4xl font-bold text-center mb-4 shrink-0" style={{ textShadow: "0px 4px 10px rgba(0,0,0,0.5)" }}>
      ...
      {!showWinner && (
        <div className="flex justify-center mb-6 h-12 shrink-0">
          {currentVoterId ? (
            <div key={currentVoterId} className="flex items-center gap-4 bg-black/50 px-6 py-2 rounded-full border border-white/20 animate-in fade-in slide-in-from-top-4 duration-500">
              <img src={sortedPlayers[currentVoterIndex]?.avatar?.url || "/placeholder.png"} className="w-8 h-8 rounded-full border-2 border-white object-cover" alt="Avatar" />
              <span className="text-lg font-bold">{sortedPlayers[currentVoterIndex]?.name} is voting...</span>
            </div>
          ) : (
            <div className="text-white/50 italic flex items-center h-full">Waiting for host to start the ceremony...</div>
          )}
        </div>
      )}

      <div className={`flex-1 min-h-0 w-full max-w-7xl mx-auto ${showWinner && videoEnded ? "mb-36" : ""}`}>
          ...
          {calculatedEntries.map((entry, index) => (
            <div 
              ...
              className={clsx(
                ...
                index === 0 ? "bg-linear-to-r from-yellow-600/40 to-yellow-900/40 border-yellow-500/50" : "bg-black/40 border-white/10"
              )}
            >
              <div className="w-8 text-xl font-black text-white/30 text-right mr-3 font-mono">{index + 1}</div>
              <div className="flex-1 z-10 flex flex-col justify-center overflow-hidden">
                ...
                <div className="text-xs opacity-60 truncate mt-0.5">{entry.artist}</div>
              </div>
              
              <div 
                className="absolute left-0 top-0 bottom-0 bg-white/5 z-0 origin-left transition-all duration-1000 ease-out" 
                style={{ width: `${calculatedEntries[0]?.points > 0 ? (entry.points / calculatedEntries[0].points) * 100 : 0}%` }} 
              />
              ...
              {entry.currentPointsToAdd && (
                <div 
                  className={clsx(
                    ...
                    entry.currentPointsToAdd === 12 ? "bg-yellow-500 text-black animate-pulse" : "bg-purple-600 text-white"
                  )}
                >
                  +{entry.currentPointsToAdd}
                </div>
              ...
              <div key={entry.points} className="text-2xl md:text-3xl font-mono font-bold w-16 text-right z-10 shrink-0 animate-in fade-in zoom-in-90 duration-300">
                {entry.points}
              </div>
            </div>
          ))}
...
{isHost && !showWinner && (
        <div className="shrink-0 bg-black/80 p-4 rounded-2xl border border-white/20 flex justify-between items-center z-10 w-full max-w-7xl mx-auto mt-4">
          <div className="text-white font-mono flex flex-col">
            ...
            <span className="font-bold">{currentVoterId ? `Voting: ${sortedPlayers[currentVoterIndex]?.name}` : "Waiting to start..."}</span>
          </div>
          <div className="flex gap-4">
            {!currentVoterId && <Button onClick={() => setNextVoter(roomCode, sortedPlayers[0].id)}>Start Presentation</Button>}
            {currentVoterId && (
              <>
                <button onClick={() => revealTwelve(roomCode)} className="px-4 py-2 bg-yellow-600/20 hover:bg-yellow-600 text-yellow-500 hover:text-white border border-yellow-600 rounded font-bold text-xs uppercase transition-colors" disabled={twelveRevealed}>Force Reveal 12</button>
                <Button onClick={() => { const nextIndex = currentVoterIndex + 1; const isFinished = nextIndex >= sortedPlayers.length; if (isFinished) finishGame(roomCode); else setNextVoter(roomCode, sortedPlayers[nextIndex].id); }} disabled={!twelveRevealed}>
                  {currentVoterIndex + 1 >= sortedPlayers.length ? "Reveal Winner" : `Next Voter: ${sortedPlayers[currentVoterIndex + 1]?.name}`}
                </Button>
              </>
            )}
          </div>
      ...
      {currentPlayerId && currentPlayerId === currentVoterId && !twelveRevealed && !showWinner && (
